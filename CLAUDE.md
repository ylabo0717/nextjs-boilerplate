...
All code must use TSDoc-style comments for better documentation, type-safety, and IDE support. TSDoc is the official TypeScript documentation standard that builds on JSDoc syntax with TypeScript-specific enhancements.

#### Documentation Generation

- **typedoc** is used to generate API documentation from TSDoc comments
- Documentation is generated to `docs/api/` directory
- Generate docs with: `pnpm docs:generate`
- Watch mode available with: `pnpm docs:watch`
...
Use TSDoc comments with `@remarks` for detailed explanations:
...
export const CONFIG = {
   ...
   * 
   * @remarks
   * Used for all REST API calls to the backend service
   */
   ...
   * 
   * @remarks
   * Applied to all HTTP requests to prevent hanging connections
   * Unit: milliseconds
   */
  TIMEOUT: 5000,
} as const;
...
interface User {
   ...
   * 
   * @remarks
   * Generated by the database upon user creation
   */
  id: string;
   ...
   * 
   * @remarks
   * Used in UI components and displayed to other users
   */
  name: string;
...
### TSDoc Tags and Modifiers

Use these standard TSDoc tags for better documentation:

- `@public` - Marks APIs intended for use by third-party developers
- `@internal` - Marks APIs intended only for use within the package
- `@param` - Documents a function parameter
- `@returns` - Documents a function's return value
- `@example` - Provides usage examples
- `@remarks` - Provides detailed explanations
- `@deprecated` - Marks deprecated APIs with migration guidance
- `@defaultValue` - Specifies the default value for optional parameters
- `@throws` - Documents exceptions that may be thrown
- `@see` - References related APIs or external resources

### Best Practices

1. **Use TSDoc for all exported APIs**: Functions, classes, interfaces, types, and constants
2. **Include @public/@internal modifiers**: Clearly indicate the intended audience
3. **Provide examples**: Use `@example` blocks for complex functions
4. **Document units**: Specify units for numeric values (milliseconds, pixels, etc.)
5. **Use @remarks for context**: Explain why something exists or how it should be used
6. **Keep descriptions concise**: First line should be a brief summary
7. **Update documentation with code changes**: Ensure comments stay in sync
8. **Avoid redundant comments**: Don't restate what the code obviously does

### Linting

- **eslint-plugin-tsdoc** validates TSDoc syntax
- TSDoc syntax errors will cause builds to fail
- Run `pnpm lint` to check for documentation issues
