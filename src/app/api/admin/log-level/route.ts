...
function createAdminAuthConfig(): AdminAuthConfig {
  ...
  const allowedOrigins = process.env.ADMIN_ALLOWED_ORIGINS?.split(',').filter(Boolean) || ['http://localhost:3000'];
 ...
 */
function checkRateLimit(clientId: string, config: AdminAuthConfig): { allowed: boolean; resetTime?: number } {
  const now = Date.now();
 ...
 */
function validateAdminAuth(request: NextRequest, config: AdminAuthConfig): { valid: boolean; error?: string } {
